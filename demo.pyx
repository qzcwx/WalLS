...
def main():
    ...
    if opt.probName == 'SAT':
        ...
        if opt.probName == 'NK':
            ...
            model = nkq.NKQLandscape(opt.n, opt.k, opt.c, opt.q, opt.t, prefixNKQ+opt.probName+'-N'+str(opt.n)+'-K'+str(opt.k)+'-C'+str(opt.c)+'-I'+str(opt.inst)+'-Q'+str(opt.q)+'-T'+str(t))
    ...
    #    print np.mean(r)
        ...
        for i in range(runs):
            # print 'run', i 
            start = time.time()
            res.append(algo.run(opt.fitName, minimize = True, restart = True,compM = opt.compMeth, beamWidth=opt.w, walklen = opt.l, radius = opt.radius))
            tAll[i] = time.time() - start
